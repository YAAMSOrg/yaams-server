...
- **Generation** — `php artisan scribe:generate`. The generated output (`resources/views/scribe/`, `public/vendor/scribe/`, `.scribe/`, `storage/app/scribe/`) is **gitignored**, so it must be (re)generated wherever the app runs or `/docs` 404s. In the dev stack this is automatic: the one-shot `docs` service in `Docker/docker-compose.yml` runs `scribe:generate` before `app` serves (like the `migrate` one-shot). For a non-Docker/production deploy, run `php artisan scribe:generate` as a deploy step (alongside `queue:restart`). Regenerate manually with `docker exec yaams-dev-app php artisan scribe:generate` after editing annotations.
