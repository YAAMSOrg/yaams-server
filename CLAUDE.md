...
- **Moderation states** live in the `status` column (`AircraftImage::STATUS_PENDING` / `STATUS_APPROVED`, plus `approved_at`/`approved_by`). `Aircraft::approvedImages()` (primary-first) is the public gallery; `Aircraft::pendingImages()` is the review queue; `Aircraft::primaryImage()` is the single primary (always approved). `Aircraft::images()` is every image regardless of state (used for the per-aircraft cap).
- **Storage is private + streamed.** Files live on the `local` (private) disk under `aircraft/{id}/{uuid}.webp` — never the `public` disk. The bytes only leave the server through the authorized `aircraft.images.show` route (`AircraftImageController::show`), gated by `AircraftPolicy::view` (member of the owning airline via the active-airline session). A **pending** image is additionally restricted to its uploader and to Managers, so it isn't visible to other members before approval.
- **Authorization (per-airline policy, not the `edit aircraft` Spatie permission).** `store` authorizes `AircraftPolicy::uploadImage` (any member of the owning airline, not retired). `approve`/`setPrimary` authorize `AircraftPolicy::update` (per-airline **Manager** + ownership + not retired). `destroy` allows a Manager (any image) **or** the uploader (their own, pending or approved). These routes deliberately do **not** use the `can:edit aircraft` middleware the older edit/retire routes carry: founding an airline / redeeming a Manager invite only grants the *per-airline* role, not the global Spatie `edit aircraft` permission, so that middleware would lock out real per-airline members.
- **Validation + re-encode = the security boundary.** `App\Support\AircraftImageProcessor` builds the settings-driven validation rules (`image`, `mimes:jpeg,jpg,png,webp`, `max:` filesize, `dimensions:max_width/height`) and re-encodes every upload through GD (Intervention Image v3) to WebP, which strips EXIF and any embedded payload. The raw upload is never stored; only the re-encoded WebP is. Over-size/over-dimension uploads are rejected (the `dimensions` rule also guards against decompression bombs). Limits are the `aircraft_image_max_*` settings above; the per-aircraft cap counts pending + approved so the queue can't be flooded.
- **Primary invariant.** Only an approved image can be primary. The first approved image (a Manager's upload, or the first pilot upload a Manager approves) becomes primary; `setPrimary` refuses pending images and flips the flag in a transaction; deleting the primary promotes the most-recent remaining **approved** image.
- **Notifications.** A pilot upload sends `App\Notifications\AircraftScreenshotSubmitted` to the owning airline's Managers (in-app always, email if the recipient opted in) via the standard `QueuesMailChannel` pattern, so the queue is discoverable without a separate review page.
- **UI** lives in a shared partial `resources/views/fleet/_gallery.blade.php`: an approved lightbox gallery (all members), a pending review section (Managers see all; a member sees only their own pending uploads), and an upload form (any member). Included on the detail view (view-only gallery + upload) and the edit view (full moderation). `AircraftResource` exposes `primaryImageUrl` (the `show` route for the primary image, or null).
